...
st.set_page_config(page_title="F1 Race Intelligence", page_icon="🏎", layout="wide")
...
with tab1:
    ...
    st.write(
        "Insira os parâmetros de corrida do piloto para calcular a probabilidade de terminar no Top 10."
    )
    ...
    with col2:
        ...
        is_rain = st.selectbox(
            "Condição Climática",
            options=[0, 1],
            format_func=lambda x: "Pista Molhada 🌧️" if x == 1 else "Pista Seca ☀️",
        )
    ...
    if run_sim:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.session_state.top10_history.append(
                    {
                        "Rótulo": driver_label
                        if driver_label
                        else f"Cenário {len(st.session_state.top10_history) + 1}",
                        "Grid": grid_pos,
                ...
                st.error(
                    f"Erro na API ({response.status_code}): {response.json().get('message')}"
                )
...
with tab2:
    ...
    st.write(
        "Consulte e compare os coeficientes de degradação calculados pelos modelos de regressão."
    )
        ...
        if st.button("Analisar Composto"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    m3.metric(
                        f"Tempo Estimado (Volta {num_laps_sim})",
                        f"{base_pace + deg_rate * num_laps_sim:.2f}s",
                    )

                    laps = list(range(1, num_laps_sim + 1))
                    lap_times = [base_pace + (deg_rate * lap) for lap in laps]

                    fig = px.line(
                        x=laps,
                        y=lap_times,
                        labels={
                            "x": "Idade do Pneu (Voltas)",
                            "y": "Tempo de Volta Estimado (s)",
                        },
                        title=f"Evolução do Tempo de Volta — Composto {compound_choice}",
                    )
                    fig.update_traces(
                        line_color=compound_colors.get(compound_choice, "#333"),
                        line_width=3,
                    )
        ...
        if st.button("Comparar SOFT vs HARD"):
            try:
                ...
                if resp_soft.status_code == 200 and resp_hard.status_code == 200:
                    ...
                    laps = list(range(1, num_laps_sim + 1))
                    soft_times = [
                        data_soft["base_pace_seconds"]
                        + data_soft["degradation_per_lap_seconds"] * lap
                        for lap in laps
                    ]
                    hard_times = [
                        data_hard["base_pace_seconds"]
                        + data_hard["degradation_per_lap_seconds"] * lap
                        for lap in laps
                    ]
                    ...
                    if len(crossover_idx) > 0:
                        ...
                        st.success(
                            f"A partir da volta **{crossover_idx[0] + 1}**, o composto HARD se torna mais rápido que o SOFT devido à degradação."
                        )
                    else:
...
with tab3:
    ...
    st.write(
        "Envie a telemetria agregada para identificar o perfil do piloto via K-Means."
    )
    ...
    with c3:
        test_label = st.text_input(
            "Nome/Identificador do Piloto", value="", key="cluster_label"
        )

    profiles = {
        0: (
            "🟢 Cluster 0: Alta Consistência e Ritmo Forte",
            "Pilotos de Elite/Líderes",
        ),
        1: ("🔴 Cluster 1: Ritmo Lento e Fundo de Grid", "Pilotos com tempos altos"),
        2: (
            "🟡 Cluster 2: Ritmo Intermediário, mas Inconsistente",
            "Pilotos Agressivos/Oscilantes",
        ),
    }
    ...
    if run_cluster:
        ...
        try:
            response = requests.post(
                f"{API_URL}/analytics/drivers/cluster", json=payload
            )
            if response.status_code == 200:
                ...
                st.session_state.cluster_history.append(
                    {
                        "Piloto": test_label
                        if test_label
                        else f"Piloto {len(st.session_state.cluster_history) + 1}",
...
with tab4:
    ...
    st.write(
        "Compare dois cenários de corrida completos, incluindo previsão de pontuação e cluster de pilotagem."
    )
    ...
    with scenario_col1:
        ...
        a_rain = st.selectbox(
            "Clima A",
            [0, 1],
            format_func=lambda x: "Molhada 🌧️" if x else "Seca ☀️",
            key="a_rain",
        )
    ...
    if st.button("Comparar Cenários", type="primary"):
        ...
        try:
            ...
            if resp_a.status_code == 200 and resp_b.status_code == 200:
                ...
                res_col1.metric(
                    "Probabilidade A", f"{data_a['probability_percentage']}%"
                )
                res_col2.metric(
                    "Probabilidade B", f"{data_b['probability_percentage']}%"
                )
                ...
                fig_compare = go.Figure(
                    data=[
                        go.Bar(
                            name="Cenário A",
                            x=["Probabilidade Top 10 (%)"],
                            y=[data_a["probability_percentage"]],
                            marker_color="#3498db",
                        ),
                        go.Bar(
                            name="Cenário B",
                            x=["Probabilidade Top 10 (%)"],
                            y=[data_b["probability_percentage"]],
                            marker_color="#e67e22",
                        ),
                    ]
                )
                fig_compare.update_layout(
                    title="Cenário A vs Cenário B",
                    barmode="group",
                    bargap=0.8,
                    bargroupgap=0.1,
                )
                ...
                if data_a["probability_percentage"] > data_b["probability_percentage"]:
                    st.success(
                        "🔵 O **Cenário A** apresenta maior probabilidade de pontuação."
                    )
                elif (
                    data_b["probability_percentage"] > data_a["probability_percentage"]
                ):
                    st.success(
                        "🟠 O **Cenário B** apresenta maior probabilidade de pontuação."
                    )
                else:
...
with tab5:
    ...
    st.write(
        "Simule a evolução volta-a-volta de corridas comparando 1 e 2 paradas nos boxes."
    )

    total_race_laps = st.slider("Total de Voltas da Corrida", 20, 78, 58)
    pit_loss = st.number_input(
        "Tempo Perdido por Pit Stop (segundos)", value=22.0, step=0.5
    )
    ...
    with strat_col1:
        ...
        st1_c1 = st.selectbox(
            "Stint 1 (Composto)", ["SOFT", "HARD"], index=0, key="s1_c1"
        )
        st1_laps1 = st.slider(
            "Voltas no Stint 1",
            1,
        ...
        st1_c2 = st.selectbox(
            "Stint 2 (Composto)", ["SOFT", "HARD"], index=1, key="s1_c2"
        )

    with strat_col2:
        st.markdown("#### 🔵 Estratégia 2: 2 Paradas")
        st2_c1 = st.selectbox(
            "Stint A (Composto)", ["SOFT", "HARD"], index=0, key="s2_c1"
        )
        st2_laps_a = st.slider(
            "Voltas Stint A",
            1,
            total_race_laps - 2,
            total_race_laps // 3,
        ...
        st2_c2 = st.selectbox(
            "Stint B (Composto)", ["SOFT", "HARD"], index=1, key="s2_c2"
        )
        ...
        try:
            ...
            if resp_soft.status_code == 200 and resp_hard.status_code == 200:
                ...
                for lap in range(1, st1_laps1 + 1):
                    t = (
                        tire_data[st1_c1]["base_pace_seconds"]
                        + tire_data[st1_c1]["degradation_per_lap_seconds"] * lap
                    )
                    if lap == st1_laps1:
                ...
                for lap in range(1, st1_laps2 + 1):
                    t = (
                        tire_data[st1_c2]["base_pace_seconds"]
                        + tire_data[st1_c2]["degradation_per_lap_seconds"] * lap
                    )
                    laps_s1_times.append(t)
                ...
                for lap in range(1, st2_laps_a + 1):
                    t = (
                        tire_data[st2_c1]["base_pace_seconds"]
                        + tire_data[st2_c1]["degradation_per_lap_seconds"] * lap
                    )
                    if lap == st2_laps_a:
                        t += pit_loss
                ...
                for lap in range(1, st2_laps_b + 1):
                    t = (
                        tire_data[st2_c2]["base_pace_seconds"]
                        + tire_data[st2_c2]["degradation_per_lap_seconds"] * lap
                    )
                    if lap == st2_laps_b:
                        t += pit_loss
                    laps_s2_times.append(t)
                ...
                for lap in range(1, st2_laps_c + 1):
                    t = (
                        tire_data[st2_c3]["base_pace_seconds"]
                        + tire_data[st2_c3]["degradation_per_lap_seconds"] * lap
                    )
                    laps_s2_times.append(t)
                ...
                m3.metric(
                    "Diferença Final",
                    f"{abs(delta):.1f}s",
                    delta=f"{'2 Paradas mais rápida' if delta > 0 else '1 Parada mais rápida'}",
                )
                ...
                df_race = pd.DataFrame(
                    {
                        ...
                        "Estratégia": ["1 Parada"] * total_race_laps
                        + ["2 Paradas"] * total_race_laps,
                    }
                )
                ...
                gap = (
                    cum_s1 - cum_s2
                )  # Posição relativa: valores > 0 significam que 2 Paradas está na frente

                df_gap = pd.DataFrame(
                    {"Volta": race_laps, "Vantagem da 2 Paradas (s)": gap}
                )

                fig_gap = px.area(
                    df_gap,
                    x="Volta",
                    y="Vantagem da 2 Paradas (s)",
                ...
                if delta > 0:
                    st.success(
                        f" A estratégia de **2 paradas** é mais rápida em {abs(delta):.1f} segundos no tempo total da corrida."
                    )
                else:
                    st.success(
                        f" A estratégia de **1 parada** é mais rápida em {abs(delta):.1f} segundos no tempo total da corrida."
                    )
...
st.divider()
st.caption(
    "F1 Race Intelligence • Painel construído com Streamlit + Plotly • Backend em Flask"
)
