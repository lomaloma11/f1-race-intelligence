...
class PredictionService:
    ...
    def predict_top10(self, data: dict):
        ...
        avg_lap_time_early = data.get("avg_lap_time_early", 90.0)
        std_lap_time_early = data.get("std_lap_time_early", 1.0)
        is_rainy = data.get("is_rainy", 0)

        # Faz a predição usando a classe original
        result = self.predictor.predict_top10(
            grid_position, avg_lap_time_early, std_lap_time_early, is_rainy
        )
        return result
