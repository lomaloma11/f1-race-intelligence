...
class DriverClusterInput(BaseModel):
    ...
    def validate_avg_lap_time(cls, v):
        if not (50.0 <= v <= 200.0):
            raise ValueError(
                "O tempo médio por volta deve estar entre 50.0 e 200.0 segundos."
            )
        return v
    ...
    def validate_std_lap_time(cls, v):
        if not (0.0 <= v <= 20.0):
            raise ValueError(
                "O desvio padrão do tempo por volta deve estar entre 0.0 e 20.0 segundos."
            )
        return v
...
class TireCompoundInput(BaseModel):
    ...
    def validate_compound(cls, v):
        ...
        if normalized not in ("SOFT", "HARD"):
            raise ValueError(
                "Composto de pneu não suportado. Os compostos suportados são: SOFT e HARD."
            )
        return normalized
