...
class BaseCollector:
    ...
    """
    def __init__(self, dataset_name: str, years: list[int], modes: list[str], output_dir: str = "data/raw"):
    ...
    def process_session(self, year: int, round_num: int, mode: str) -> bool:
        ...
        try:
            ...
            print(f" Erro extraindo [{self.dataset_name}] em {year} R{round_num:02d} ({mode}): {e}")
            return False
        ...
        print(f" [{self.dataset_name.upper()}] Salvo: {year} | Round {round_num:02d} | Modo {mode}")
        return True
            ...
            for round_num in range(1, 30):
                for mode in self.modes:
                    ...
                    if not success and mode == "R":
                        print(f" Fim da temporada {year} atingido no round {round_num - 1}.")
                        break
                else:
                    time.sleep(0.5)
                    continue
...
def get_cli_args():
    ...
    parser.add_argument("--years", "-y", nargs="+", type=int, help="Lista de anos (ex: -y 2021 2022 2023)")
    parser.add_argument("--modes", "-m", nargs="+", default=["R", "S"], help="Sessoes (ex: R S Q)")
